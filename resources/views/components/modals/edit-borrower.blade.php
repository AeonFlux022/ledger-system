...
<div x-data="{ open: false }" x-key="borrower-{{ $borrower->id }}">
  <!-- Trigger -->
  ...
  <div x-show="open" x-cloak x-transition.opacity
    class="fixed inset-0 bg-black/50 flex items-start justify-center py-10 z-50">
    <!-- Modal Card -->
    <div @click.outside="open = false" x-transition.scale
      class="bg-white w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 rounded shadow-lg">
      ...
      <form method="POST" action="{{ route('admin.borrowers.update', $borrower->id) }}" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="number" name="income" value="{{ $borrower->income }}" class="w-full border px-3 py-2 rounded">
          </div>
          ...
          <div>
            ...
            @if ($borrower->id_image)
              <img src="{{ asset($borrower->id_image) }}" class="h-32 object-contain rounded mb-2">
            @else
